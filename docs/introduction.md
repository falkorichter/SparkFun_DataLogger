...
To configure the DataLogger, a menu system is accessed via a serial connection and provides access to all available settings for the system. Changes are automatically saved in persistant storage on the device, and are also settable/accessable as a `json` file if desired.  Details on setting up the system are outlined in the [Operation Overview](system-operation.md) section of this manual.
